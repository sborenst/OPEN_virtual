...
* Download and install https://www.vagrantup.com/downloads.html[Vagrant]
...
NOTE: Alternatively, you could also add it usign these instructions - https://gist.github.com/fernandoaleman/5083680
...
For performances reason, we will use NFS between the Host and Guest machine to copy the VirtualBox Guest extensions iso file, JBoss Fuse or any other files. To automate the procedure and avoid that Vagra,t requests to the host to provide the admin password, the following lines will be added to the sudoers file of the host.
...
If the image (ova file) has been imported, then you can start the VM machine and issue a ssh session
