...
virt-install --connect qemu:///system --name=fuse-dev --virt-type=kvm --ram=2048 --os-type=linux --os-variant=fedora22 --disk path=fuse-dev_libvirt.qcow2,device=disk,format=qcow2 --vcpus=2 --network bridge=virbr0 --vnc --noautoconsole --import --check all=off
---- 
