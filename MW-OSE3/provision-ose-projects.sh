...
declare -A COURSES
COURSES=(["DEVOPS_USING_OSE"]=1 ["BPMS"]=2 ["JDV_DEV"]=2 ["AMQ"]=1)
...
verify_course() {
    ...
    # https://github.com/redhat-gpe/app_dev_with_ose/issues/109
    if [ "$COURSENAME" = "$DEVOPS_USING_OSE" ] ; then
        COURSENAME="OSEDEV"
    fi
}
